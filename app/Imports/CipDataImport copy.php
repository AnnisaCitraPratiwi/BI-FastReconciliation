...
class CipDataImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        for ($rowIndex = $startRow; $rowIndex < count($rowsArray); $rowIndex++) {
            ...
            if (empty($transactionId) || $transactionId === 'Beginning Balance') {
                continue;
            }
            ...
            // Extract tanggal dari Transaction ID (6 digit pertama)
            $transactionDate = $this->extractDateFromTransactionId($transactionId);
            
            if (!$transactionDate) {
                Log::warning("Skip row {$rowIndex} - cannot extract transaction date from: {$businessMsgId}");
                continue;
            }
