{{-- resources/views/reconciliation/history-view.blade.php --}}
    ...
    <div class="flex-grow-1">
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-white border-bottom px-4">
            <div class="d-flex flex-column">
                <span class="navbar-brand mb-0 h1">View Reconciliation Results</span> 
            </div>
                        ...
                        <div class="modal-body">
                            <div class="text-center mb-4">
                                <img id="profilePreview"
                                    src="{{ Auth::user()->profile_photo ? asset('storage/profile_photos/' . Auth::user()->profile_photo) : asset('assets/images/profile-default.jpg') }}"
                                    class="rounded-circle" alt="User" style="width: 100px; height: 100px; object-fit: cover;">
                                <div>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('reconciliation.history') }}" class="btn btn-secondary">
                            ...
                            <tbody id="anomaliTableBody">
                                @forelse($anomalies as $item)
                                    @php
                                        ...
                                        if (!empty($ams['trx_date_time'])) {
                                            $trxTime = \Carbon\Carbon::parse($ams['trx_date_time'])->setTimezone('Asia/Jakarta')->format('H:i:s');
                                        } elseif (!empty($cip['transaction_date'])) {
                                            $trxTime = \Carbon\Carbon::parse($cip['transaction_date'])->setTimezone('Asia/Jakarta')->format('H:i:s');
                                        } else {
                                            $trxTime = '-';
                                        }  

                                        $trxSource = $ams['trx_source'] ?? '-';
                                        $srcAccount = $ams['source_account_number'] ?? $cip['rekening_pengirim'] ?? '-';
                                        $dstAccount = $ams['destination_account_number'] ?? $cip['rekening_penerima'] ?? '-';

                                        $nominal = (!empty($cip['debit']) ? $cip['debit'] : 0) + (!empty($cip['kredit']) ? $cip['kredit'] : 0);
                                        if (!$nominal) $nominal = !empty($ams['trx_amount']) ? $ams['trx_amount'] : 0;
                                        if (!$nominal) $nominal = !empty($bs['nilai_transaksi']) ? $bs['nilai_transaksi'] : 0;
                                        $totalNominal += floatval($nominal);

                                        $statusInline = '<span class="me-2" title="CIP"><strong>CIP</strong> ' . ($cip ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-times-circle text-danger"></i>') . '</span>';
                                        $statusInline .= '<span class="me-2" title="AMS"><strong>AMS</strong> ' . ($ams ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-times-circle text-danger"></i>') . '</span>';
                                        $statusInline .= '<span title="BS"><strong>BS</strong> ' . ($bs ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-times-circle text-danger"></i>') . '</span>';
                                        ...
                                        if ($cip && $ams && $bs)         $keterangan = 'Data Lengkap';
                                        elseif (!$cip && !$ams && $bs)   $keterangan = 'Hanya Ditemukan di BS';
                                        elseif (!$cip && $ams && !$bs)   $keterangan = 'Hanya Ditemukan di AMS';
                                        elseif ($cip && !$ams && !$bs)   $keterangan = 'Hanya Ditemukan di CIP';
                                        elseif ($cip && $ams && !$bs)    $keterangan = 'Tidak Ditemukan di BS';
                                        elseif (!$cip && $ams && $bs)    $keterangan = 'Tidak Ditemukan di CIP';
                                        elseif (isset($item['type']) && $item['type'] === 'AMOUNT_MISMATCH') $keterangan = 'Beda Nominal';
                                    @endphp
                                    <tr>
                                        <td class="text-center">{{ $loop->iteration }}</td>
                                        <td><small>{{ $endToEndId }}</small></td>
                                        <td><small>{{ $referenceNumber }}</small></td>
                                        <td>{{ $trxTime }}</td>
                                        <td><small>{{ $trxSource }}</small></td>
                                        <td><small>{{ $srcAccount }}</small></td>
                                        <td><small>{{ $dstAccount }}</small></td>
                                        <td class="fw-bold text-end">{{ number_format($nominal, 0, ',', '.') }}</td>
